...
def _replace_tree_with_list_in_views(logger,base_path,module_name=None,dry_run=False):
...
def _replace_chatter_blocks(logger,base_path,module_name=None,dry_run=False):
    """Convert odoo.osv.expression usage to odoo.fields.Domain"""
    if module_name:
        module_path = module_name
    else:
...
def _replace_deprecated_kanban_box_card_menu(logger,base_path,module_name=None,dry_run=False):
    """Convert odoo.osv.expression usage to odoo.fields.Domain"""
    if module_name:
        module_path = module_name
    else:
...
def _replace_user_has_groups(logger,base_path,module_name=None,dry_run=False):
    """Convert odoo.osv.expression usage to odoo.fields.Domain"""
    if module_name:
        module_path = module_name
    else:
...
def _replace_unaccent_parameter(logger,base_path,module_name=None,dry_run=False):
    """Convert odoo.osv.expression usage to odoo.fields.Domain"""
    if module_name:
        module_path = module_name
    else:
...
def _replace_ustr(logger,base_path,module_name=None,dry_run=False):
    """Convert odoo.osv.expression usage to odoo.fields.Domain"""
    if module_name:
        module_path = module_name
    else:
...
def _replace_editable_attribute(logger,base_path,module_name=None,dry_run=False):
    """Convert odoo.osv.expression usage to odoo.fields.Domain"""
    if module_name:
        module_path = module_name
    else:
...
def migrate_file_v170_v180(env, base_path, module_name=None) -> bool:
    _replace_tree_with_list_in_views(env._logger, base_path, module_name, env.dry_run)
    _replace_chatter_blocks(env._logger, base_path, module_name, env.dry_run)
    _replace_deprecated_kanban_box_card_menu(env._logger, base_path, module_name, env.dry_run)
    _replace_user_has_groups(env._logger, base_path, module_name, env.dry_run)
    _replace_unaccent_parameter(env._logger,base_path, module_name,env.dry_run)
    _replace_ustr(env._logger,base_path,module_name,env.dry_run)
    _replace_editable_attribute(env._logger,base_path,module_name,env.dry_run)
    return True

PythonTransformer.migrate_file_v170_v180 = migrate_file_v170_v180